...
c-----------------------------------------------------------------------
      ...
      integer np
      real(kind=star10) rp(3,np),cm(np),rpi(3),cmimin,cmimax,tol
...
c        local variables
      ...
      integer km,kp
      logical inmax,inmin
      real(kind=star10) cmi,cmik,cmim,dph,ph,phm,php,phimax,phimin,
     *  si,sik,xi(3),yi(3)
c *
...
c        cmik = 1-cos th(ik), th(ik)=angle twixt rpi & rp(i)
     ...
     *    +(rpi(3)-rp(3,i))**2)/2._star10
c        sik = sin th(ik)
        sik=sqrt(cmik*(2._star10-cmik))
c        min circle is outside area
        if ((cm(i).ge.0._star10.and.cmik.ge.cmi)
     *    .or.(cm(i).lt.0._star10.and.cmik.le.cmi)) inmin=.false.
c        max circle is outside area
        if ((cm(i).ge.0._star10.and.cmik.le.2._star10-cmi)
     *    .or.(cm(i).lt.0._star10.and.cmik.ge.2._star10-cmi)) inmax=.false.
...
c        segment contains nearest point in i circle, phi=0
              if (phm.le.0._star10.and.php.ge.0._star10) phimin=0._star10
            elseif (php.lt.phm) then
c        segment contains nearest point in i circle, phi=0
              if (phm.le.0._star10.or.php.ge.0._star10) phimin=0._star10
