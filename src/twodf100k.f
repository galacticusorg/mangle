...
c-----------------------------------------------------------------------
      function twodf100k(ra,dec)
      integer, parameter ::  star10 = selected_real_kind(r=4931,p=18)
      real(kind=star10) ra,dec,twodf100k
c
c        parameters
      include 'pi.par'
      real(kind=star10) RADIAN
      parameter (RADIAN=180._star10/PI)
c        data variables
C     logical init
C     real magcut
...
c convert from real(kind=star10) degrees to real*4 radians
      rar=ra
      decr=dec
...
c completeness: round to 8 decimal places
      twodf100k=dble(nint(twodf100k*1.e8_star10))/1.e8_star10
...
c COMMENT THIS OUT IF YOU DON'T WANT IT
        inquire(FILE=name_mask, EXIST=ex)
        if (.not.ex) then
...
c COMMENT THIS OUT IF YOU DON'T WANT IT
         inquire(FILE=namemask, EXIST=ex)
         if (.not.ex) then
