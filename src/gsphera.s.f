...
c-----------------------------------------------------------------------
     ...
     *  azmin,azmax,elmin,elmax,v,dw)
      ...
      real(kind=star10) area,bound(2),vert(2),w(im,nw),
     *  azmin,azmax,elmin,elmax,dw(nw)
c        work array (could be automatic if compiler supports it)
      real(kind=star10) v(lmax1)
c
c        parameters
      include 'pi.par'
      real(kind=star10) TWOPI,PIBYTWO
      parameter (TWOPI=2._star10*PI,PIBYTWO=PI/2._star10)
c        data variables
      real(kind=star10) elmino,elmaxo
c        saved variables
      real(kind=star10) cl,cu,dth,sl,su
      save cl,cu,dth,sl,su
!$omp threadprivate(cl,cu,dth,sl,su)
c        local (automatic) variables
      integer i,l,m,lm,lmax,mmax
      real(kind=star10) azmx,cmph,d,dph,ph,smph,thmin,thmax
c *
...
c
      data elmino,elmaxo /2*0._star10/
c
c        zero stuff
      area=0._star10
      bound(1)=0._star10
...
c--------fast computation of harmonics
      ...
      if (ibv.eq.0.or.ibv.eq.2.or.ibv.eq.3) then
        bound(1)=(sl+su)*dph+2._star10*dth
        bound(2)=(1._star10/sl-2._star10*sl+1._star10/su-2._star10*su)*dph-2._star10*dth
        vert(1)=4._star10
        vert(2)=4._star10*(cl/sl-cu/su)
        if (ibv.eq.2) then
          bound(1)=-bound(1)
          bound(2)=-bound(2)
          vert(1)=-vert(1)
      ...
      ph=(azmx+azmin)/2._star10
      if (ibv.ge.2) dph=-dph
      lmax=lmax1-1
      mmax=lmax
      if (dph-nint(dph/TWOPI)*TWOPI.eq.0._star10) mmax=0
      do m=0,mmax
        if (m.eq.0) then
          d=dph
        elseif (m.gt.0) then
          d=sin(m*dph/2._star10)*2._star10/dble(m)
        endif
        cmph=cos(m*ph)
        smph=sin(m*ph)
        lm=(m*(m+1))/2+1
