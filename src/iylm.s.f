c-----------------------------------------------------------------------
      ...
      integer lmax1,nw
      real(kind=star10) thmin,thmax,w(nw)
...
c        parameters
      include 'pi.par'
c        data variables
      real(kind=star10) tiny
c        local (automatic) variables
      integer lm,lmx1,qphi
      real(kind=star10) ri,phi,zi,ci,si,ph,dph
c *
...
c        upper latitude term
      ...
      ph=0._star10
      dph=-tiny
      if (thmin.eq.0._star10.or.thmin.eq.PI) then
        si=0._star10
        lmx1=1
      else
        si=sin(thmin)
...
c        lower latitude term
      dph=tiny
      if (thmax.eq.0._star10.or.thmax.eq.PI) then
        si=0._star10
        lmx1=1
      else
        si=sin(thmax)
