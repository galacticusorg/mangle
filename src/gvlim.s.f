...
c-----------------------------------------------------------------------
     ...
     *  rp,cm,np,rpi,vcirc,tol,phi,iord,wk,iwk,ldegen)
      ...
      logical ldegen
      real(kind=star10) vmin(3,nvmax),vmax(3,nvmax),cmvmin(nvmax),cmvmax(nvmax),
...
c        work arrays (could be made automatic if compiler supports it)
      ...
      real(kind=star10) phi(2,np),wk(nvmax)
c
c        parameters
      include 'pi.par'
...
c        externals
      ...
      real(kind=star10) cmijf
c        data variables
      real(kind=star10) big
c     real(kind=star10) dphmin
c        local variables
      integer i,ii,ik,iphi,iseg,iv,j,jm,jml,jmu,jp,jpl,jpu,km,kp,ni,scmi
C     logical warn
      real(kind=star10) cmi,cmik,dph,ikchk,ph,phin,phif,phm,phmax,phmin,php,
