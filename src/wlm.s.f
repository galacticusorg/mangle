...
c-----------------------------------------------------------------------
      ...
      real(kind=star10) w(im,nw),ri,phi,zi,ci,si,ph,dph,v(lmax1)
c
c        parameters
      real(kind=star10) HALF
      parameter (HALF=1._star10/2._star10)
      include 'pi.par'
      real(kind=star10) TWOPI
      parameter (TWOPI=2._star10*PI)

c largest non-overflowing 2^(DBL_MAX_EXP-1) for real(kind=star10) (from values.h)
      integer DBL_MAX_EXP
...
c
      ...
      if (dph.eq.0._star10) goto 300
c        largest non-overflowing power of 2 for real(kind=star10)
      OVFLOW=2._star10**(DBL_MAX_EXP-1)
      UNFLOW=1._star10/OVFLOW
      z=OVFLOW*0._star10
      if (z.ne.0._star10.or.UNFLOW.eq.0._star10) then
...
c        done this way to ensure accuracy at small ri
      cm=-((1._star10-zi)**2+ri**2)/4._star10
      cp=((1._star10+zi)**2+ri**2)/4._star10
      nmax1=lmax1
      if (dph.eq.TWOPI.or.dph.eq.-TWOPI) nmax1=1
      if (si.eq.0._star10) nmax1=1
      mmin1=1
      mmax1=lmax1
      do 280 n1=1,nmax1
        n=n1-1
...
c........l = n = 0
          if (n.eq.0) then
            if (l.eq.0) then
c        zn=z(0,0)
              zn=1._star10/sqrt(2._star10*TWOPI)
              ezn=0
              ez=ezn
              fn=dph
c        z=z(0,0); zp=z(1,0)
...
c        zn may underflow
            ...
            ez=ezn
            zn=z
            fn=2._star10*sin(an*dph/2._star10)/an
...
c        recover from underflow
              if (abs(zp).ge.1._star10
     *          .and.(abs(z).ge.1._star10.or.z.eq.0._star10)
     *          .and.(abs(zm).ge.1._star10.or.zm.eq.0._star10)) then
                ez=ez-1
                zm=zm*UNFLOW
                z=z*UNFLOW
                zp=zp*UNFLOW
...
c        dm=d(n,m,n); em=(-)^n*d(n,m,-n)
            t=sqrt((an-am+1._star10)/(an+am))*2._star10/ri
c        dm
            d=dm*cp*t
c        dm may underflow ...
            if (abs(d).le.UNFLOW.and.dm.ne.0._star10) then
              dm=dm*OVFLOW
              d=dm*cp*t
              edm=edm+1
c        ... or may recover from underflow
            elseif (edm.gt.0) then
              if (abs(d).ge.1._star10) then
                d=d*UNFLOW
                edm=edm-1
              endif
            endif
...
c        em
            e=em*cm*t
c        em may underflow ...
            if (abs(e).le.UNFLOW.and.em.ne.0._star10) then
              em=em*OVFLOW
              e=em*cm*t
              eem=eem+1
c        ... or may recover from underflow
            elseif (eem.gt.0) then
              if (abs(e).ge.1._star10) then
                e=e*UNFLOW
                eem=eem-1
              endif
            endif
...
c        dn
            d=cp*dn
c        dn may underflow
            if (abs(d).le.UNFLOW.and.dn.ne.0._star10) then
              dn=dn*OVFLOW
              d=cp*dn
              edn=edn+1
            endif
...
c        en
            e=cm*en
c        en may underflow
            if (abs(e).le.UNFLOW.and.en.ne.0._star10) then
              en=en*OVFLOW
              e=cm*en
              een=een+1
            endif
...
c        d=d(l,m,n); e=(-)^n*d(l,m,-n)
            ...
            elseif (l.gt.mn) then
              lm=lm+l
              qq=2._star10*al-1._star10
              q1=q
              q=sqrt((al+am)*(al-am)*(al+an)*(al-an))/al
              t=0._star10
              if (m*n.gt.0) t=am*an/((al-1._star10)*al)
              d2=d1
              d1=d
              d=(qq*(zi-t)*d1-q1*d2)/q
c        recover d from underflow
              if (ed.gt.0) then
                if (abs(d).ge.1._star10
     *            .and.(abs(d1).ge.1._star10.or.e1_star10.eq.0._star10)
     *            .and.(abs(d2).ge.1._star10.or.e2_star10.eq.0._star10)) then
                  d2=d2*UNFLOW
                  d1=d1*UNFLOW
                  d=d*UNFLOW
                  ed=ed-1
...
c        recover e from underflow
                if (ee.gt.0) then
                  if (abs(e).ge.1._star10
     *              .and.(abs(e1).ge.1._star10.or.e1.eq.0._star10)
     *              .and.(abs(e2).ge.1._star10.or.e2.eq.0._star10)) then
                    e2=e2*UNFLOW
                    e1=e1*UNFLOW
                    e=e*UNFLOW
                    ee=ee-1
