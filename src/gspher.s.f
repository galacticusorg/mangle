...
c-----------------------------------------------------------------------
     ...
     *  iphi,tol,phw,iord,v,ldegen)
      ...
      logical ldegen
      real(kind=star10) area,bound(2),vert(2),w(im,nw),rp(3,np),cm(np),tol
c        work arrays (could be automatic if compiler supports it)
      integer iord(2*np)
      real(kind=star10) phw(2,np),v(lmax1)
c
c        parameters
      include 'pi.par'
...
C     logical warn
      logical whole
      real(kind=star10) bik,ci,cmi,cmik,cmk,cti,ctk,ctpsi,
...
c........ri, phi, rp(3,i) are cylindrical coordinates of rp(i)
        ...
        if (ri.eq.0._star10.or.i.eq.iphi) then
          phi=0._star10
        else
...
c        if rp(i) is along z-axis, set yi in direction z x rp(iphi)
        elseif (rii.gt.0._star10) then
          yi(1)=-rp(2,iphi)/rii
          yi(2)=rp(1,iphi)/rii
          yi(3)=0._star10
c        if rp(iphi) is also along z-axis, set yi along y-axis
        elseif (ri.eq.0._star10.and.rii.eq.0._star10) then
          yi(1)=0._star10
          yi(2)=1._star10
          yi(3)=0._star10
        endif
...
c        bound(1) term is length of boundary
          dbound(1)=si*abs(dph)
          dbound(2)=(1._star10/si-2._star10*si)*abs(dph)
c        standard
          if (ibv.eq.0
c        cross
...
c        cti = cot th(i)
              cti=(1._star10-cmi)/si
              if (scmi.lt.0) cti=-cti
c        ctk = cot th(k)
              ctk=(1._star10-cmk)/sk
              if (cm(k).lt.0._star10) ctk=-ctk
c        standard
              if (ibv.eq.0
     ...
     *                         .or.(i.le.npc.and.k.le.npc)))) then
                if (psi.eq.0._star10) then
                  dvert(1)=0._star10
                  dvert(2)=0._star10
                else
                  dvert(1)=1._star10-psi*ctpsi
                  dvert(2)=t*(3._star10+t**2)*(cti+ctk)/2._star10
                endif
...
c        cross
              elseif (ibv.eq.1) then
                if (i.le.npc.and.k.le.npc) then
                  ...
                  dvert(1)=PI/2._star10*ctpsi
                  dvert(2)=t*(3._star10+t**2)*(cti+ctk)/2._star10
                  vert(1)=vert(1)-dvert(1)
                  vert(2)=vert(2)+dvert(2)/2._star10
                  t=1._star10/t
                  dvert(2)=t*(3._star10+t**2)*(cti-ctk)/2._star10
                  if (i.gt.npc) then
                    vert(2)=vert(2)-dvert(2)/2._star10
...
c        psip = pi - psi
                  psip=atan2(d,-bik)
                  if (psip.eq.0._star10) then
                    dvert(1)=0._star10
                    dvert(2)=0._star10
                  else
                    dvert(1)=1._star10+psip*ctpsi
                    t=1._star10/t
                    dvert(2)=t*(3._star10+t**2)*(cti-ctk)/2._star10
                  endif
                  vert(1)=vert(1)-dvert(1)
...
C             print *,' cot(',psi,') =',1._star10/tan(psi),
C    *          ' should =',ctpsi
C             print *,' tan(',psi,'/2) =',tan(psi/2._star10),
C    *          ' should =',t 
