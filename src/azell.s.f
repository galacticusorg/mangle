...
c-----------------------------------------------------------------------
      ...
      real(kind=star10) rag,decg,l2p,raz,elp,azp,azg,elg,l2z
c
c        parameters
      real(kind=star10) CIRCLE,PI,RADIAN
      parameter (CIRCLE = 360._star10,
     *           PI = 3.1415926535897932384626_star10,
     *           RADIAN = 180._star10/PI)
c        local (automatic) variables
      integer iz
      real(kind=star10) cazm,cdecg,celg,celp,cl2m,cra
      real(kind=star10) sazm,sdecg,selg,selp,sl2m,sra
c *
...
c        sine and cosine of elevation of NGP
      ...
      if (selg.gt.1._star10) then
        selg=1._star10
      elseif (selg.lt.-1._star10) then
        selg=-1._star10
      endif
      celg=sqrt(1._star10-selg*selg)
c        elevation of NGP in deg
      elg=asin(selg)*RADIAN
c        at NGP el +- 90 deg, set NGP az & zenith long consistently
      if (celg.eq.0._star10) then
        azg=azp
        l2z=l2p+CIRCLE/2._star10
      elseif (celg.ne.0._star10) then
...
c        ensure azimuthal angles are in interval [0,360)
      iz=azg/CIRCLE
      if (azg.lt.0._star10) iz=iz-1
      azg=azg-iz*CIRCLE
      iz=l2z/CIRCLE
      if (l2z.lt.0._star10) iz=iz-1
      l2z=l2z-iz*CIRCLE
      return
      end
c
