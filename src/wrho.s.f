...
c-----------------------------------------------------------------------
      function wrho(az,el,w,lmax,mmax,im,nw,lsmooth,esmooth)
      integer, parameter ::  star10 = selected_real_kind(r=4931,p=18)
      integer lmax,mmax,im,nw
      real(kind=star10) el,az,w(im,nw),lsmooth,esmooth, wrho
c
c        parameters
...
c     integer i
      ...
      real(kind=star10) al,al1,am,cel,cm,dwrho,lsmoot1,phi,
...
c
      ...
      do 180 m1=1,mmax1
        ...
        if (m.eq.0) then
c        zn=z(0,0)
          zn=1._star10/sqrt(4._star10*PI)
          cm=1._star10
          sm=0._star10
        elseif (m.gt.0) then
c        zn=z(m,m)
          zn=-sqrt((am-HALF)/am)*cel*zn
          cm=2._star10*cos(am*phi)
          if (im.eq.2) sm=2._star10*sin(am*phi)
        endif
        z=0._star10
        zp=zn
        lm=(m*m1)/2+1
...
c        dwrho=w(l,m)*Y(l,m)+w(l,-m)*Y(l,-m)
          ...
          dwrho=dwrho*y
          if (lsmooth.gt.0._star10) then
            smooth=exp(-(al/lsmooth*al1/lsmoot1)**(esmooth/2._star10))
            dwrho=dwrho*smooth
          endif
          wrho=wrho+dwrho
